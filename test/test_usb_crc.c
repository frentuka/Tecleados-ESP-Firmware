 ...
 *
 * Tests CRC prepare/verify round-trip, known vectors, corruption detection,
 * and boundary conditions.
 */
#include "test_harness.h"
#include "mocks/mock_esp.h"

/* Bring in the CRC implementation directly (it's pure logic) */
#define COMM_REPORT_SIZE 63

/* Inline the CRC table and computation from usb_crc.c */
...
static void usb_crc_prepare_packet(uint8_t *packet) {
    uint8_t payload_size = COMM_REPORT_SIZE - 1;
    uint8_t crc = compute_crc8(packet, payload_size);
    packet[payload_size] = crc;
}

static bool usb_crc_verify_packet(const uint8_t *packet) {
    uint8_t crc = compute_crc8(packet, COMM_REPORT_SIZE);
    return (crc == 0);
}
...
TEST_CASE(usb_crc, every_single_byte_flip_detected) {
    ...
    /* Flip each byte individually and verify detection */
...
TEST_CASE(usb_crc, compute_crc8_empty_is_zero) {
    uint8_t crc = compute_crc8(NULL, 0);
    TEST_ASSERT_EQUAL(0, crc);
}

TEST_CASE(usb_crc, compute_crc8_single_byte) {
    uint8_t data = 0x01;
    uint8_t crc = compute_crc8(&data, 1);
    TEST_ASSERT_EQUAL(crc8_table[0x01], crc);
}
