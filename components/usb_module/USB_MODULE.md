...
The `usb_module` component implements the full USB stack for the TEF-Oxide keyboard firmware. It exposes the device to the host as a **dual-interface HID device**: one interface for keyboard input (6KRO, NKRO, consumer control) and one bidirectional vendor-defined interface used as a reliable configuration and command channel.
