...
static uint8_t const desc_hid_report_kbd[] = {
    ...
    // --- Padding ---
    0x75, 0x01,                         // Report Size (1)
    0x95, (NKRO_BYTES * 8 - NKRO_KEYS), // Report Count (padding bits)
    0x81, 0x03,                         // Input (Const,Var,Abs)
    0xC0                                // End Collection
};

// Vendor-defined HID descriptor for bidirectional config data (64 bytes input +
// output)
...
#define HID_KBD_DESC_SIZE                                                      \
  25 // TUD_HID_DESCRIPTOR (Interface 9 + HID 9 + Endpoint 7)
#define HID_COMM_DESC_SIZE                                                     \
  32 // TUD_HID_INOUT_DESCRIPTOR (Interface 9 + HID 9 + Endpoint Out 7 +
     // Endpoint In 7)

// Total configuration descriptor length
#define CONFIG_TOTAL_LEN                                                       \
  (CONFIG_DESC_SIZE + HID_KBD_DESC_SIZE + HID_COMM_DESC_SIZE)
...
static uint8_t const desc_configuration[] = {
    ...
    TUD_HID_DESCRIPTOR(ITF_NUM_HID_KBD, // bInterfaceNumber
