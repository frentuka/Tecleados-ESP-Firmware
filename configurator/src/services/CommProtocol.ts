...
// ── CRC-8 Table (polynomial 0x07, matches firmware comm_crc.c) ──────────
...
export class CommProtocol {
    private maxPacketSize = 63; // Default for USB
    ...
    constructor(io: {
        ...
        this.io.onRawReceived(this.handleInputReport);
    }

    public setMaxPacketSize(size: number) {
    ...
    // Observers (pass-through from Transport)
    ...
    private async safeSendReport(data: Uint8Array): Promise<boolean> {
        try {
            await this.io.sendRaw(data);
            return true;
        } catch (error: any) {
            console.error(`[CommProtocol] Fatal send error: ${error.name}: ${error.message}`);
            return false;
        }
    }
    ...
    private async sendPacketByIndex(data: Uint8Array, index: number, totalPackets: number): Promise<boolean> {
        const offset = index * this.maxPayloadLength;
        const chunk = data.slice(offset, offset + this.maxPayloadLength);
    ...
    private blastRxReceivePacket(index: number, payload: Uint8Array, payloadLen: number): void {
        ...
        const offset = index * this.maxPayloadLength; // use maxPayloadLength for offset to handle variable packet sizes correctly
    ...
    private blastRxAssemblePayload(): Uint8Array {
        ...
        for (let i = 0; i < this.blastRx.totalPackets; i++) {
            ...
            const offset = i * this.maxPayloadLength;
    ...
    public async sendCustomCommReport(data: Uint8Array): Promise<boolean> {
        try {
            ...
            for (let round = 0; round < MAX_RECONCILE_ROUNDS; round++) {
                ...
                if (!await this.safeSendReport(statusPacket)) {
    ...
    private async processNextTask(): Promise<void> {
        ...
        try {
            ...
            console.error('[CommProtocol Queue] Task failed:', err);
            reject(err);
        } finally {
            setTimeout(() => {
    ...
    private async handleInputReport(data: Uint8Array): Promise<void> {
        if (data.length < 5) return; // Need at least header + crc

        const flags = data[0];
        const remaining = data[1] | (data[2] << 8);
        const payloadLenFromHeader = data[3];
        const safeLen = Math.min(payloadLenFromHeader, this.maxPayloadLength);
